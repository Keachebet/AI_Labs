...
from my_classes.gbfs_madaraka import GBfsTraverser
...
def getHeuristics(G):
    ...
    f = open(os.path.join(__location__, 'my_heuristics.txt'))
    for i in G.nodes():
...
my_heuristics = getHeuristics(G)
node_pos = nx.get_node_attributes(G,'pos')

#call BFS to return set of all possible routes to the goal
route_bfs = GBfsTraverser()
routes = route_bfs.GBFS(G,my_heuristics,"SportsComplex","ParkingLot")


route_list = route_bfs.path
#color the nodes in the route_bfs
node_col = ['gray' if not node in route_list else 'gold' for node in G.nodes()]
gold_edges = list(zip(route_list,route_list[1:]))
#color the edges as well
#print(peru_colored_edges)
edge_col = ['gray' if not edge in gold_edges else 'gold' for edge in G.edges()]
